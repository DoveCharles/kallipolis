...
// Only FLOYD_CARRY of each pixel's error is passed on: error diffusion is chaotic — one pixel changing reshuffles the dots
// after it — so with all of it carried, a car moving sets the whole view flickering; damped, a change dies out nearby.
const FLOYD_STEINBERG = DITHER_PATTERNS.findIndex(p => p.id === 'floyd');
const FLOYD_CARRY = 0.8;
...
function floydSteinberg(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let n = 0; n < width; n++) {
      ...
      for (let c = 0; c < 3; c++) {
        const error = ((c === 0 ? r : c === 1 ? g : b) - floydPalette[best + c])*FLOYD_CARRY;
