...
- `pigeons.js` (33K) Flocks of pigeons on plazas and in parks (assets/models/Pigeon.glb): walk, peck, flick their heads about, scatter from people (and blasts) nearby, fly round and land together elsewhere; roost after dark. Clicked in World mode, the camera follows one and its card (assets/pigeons.txt) comes up. The rigged clips are baked to one morph target per frame, one instanced mesh per zone. Planted by zones/plazas.js and surface-detail.js. — exports: loadPigeonModel, plantPigeons, updatePigeons
