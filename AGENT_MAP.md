...
- `settings-windows.js` (1K) Under the Windows 3.0 window, the World panel's set-once settings (index.html's data-settings groups) open from the Options menu in windows of their own — Display, Effects, Game — moved in while open and put back after. — exports: openSettings
...
- `win3.js` (1K) A checkbox in World settings dresses the UI up as Windows 3.0 — the styles are all in css/win3.css, under html.win3.
- `win3-menu.js` (11K) Under the Windows 3.0 look, wider than a phone: the whole tab as the Splinetopia window — frame, title bar, File/Edit/View/Options/Help menu bar (items press the existing buttons), Alt+letter menus, Ctrl+S/Ctrl+O, message boxes (About, Clear All, shortcuts); the side panel docks down its left, and its World/Edit/Maps buttons move into the toolbar (back to the panel on phones or with the look off), where pressing the one that's down lets it up and hides the panel, leaving World's controls; tracks the active window, so a card opened on something (even one already open) takes the navy title bar and the frame's goes white. Styles in css/win3.css ("the application window").
