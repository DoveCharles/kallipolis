...
## assets/
- `text/*.txt` Editable data read by `src/core/entries.js`: `text/people/` (one file per list: `about.txt` — notes, trait table, [distribution] — `boynames`, `girlnames`, `unisexnames` (into both), `surnames`, `nicknames`, `moods`, `loves`, `hates`; joined in order under their headings by life/profiles.js PEOPLE_FILES), `cars.txt`, `buildings.txt`, `bees.txt`, `pigeons.txt`, `planes.txt`, `trains.txt`, `tv.txt` (YouTube links or direct video file URLs), `morality.txt`, `dictionary.txt` (real spoken lines).
