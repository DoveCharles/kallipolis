...
- `peopleHolding.js` (19K) Anything a person carries (the hot dog and coffee cup are assets/models/Holdables.glb), and a hot dog or coffee bought from a stall eaten/drunk mouthful by mouthful while walking, standing or sitting (snackClip picks the Walk/Idle/Sit1 variant holding it): — exports: hold, letGo, holding, serveMeal, clearMeal, mealFinished, mealCue, updateHeld, giveSnack, dropSnack, snackClip
...
- `held-debug.js` (5K) View > Held Items (debug): sliders for a hot dog's or coffee's place in the hand (ITEMS) and the hand's pose (SNACK_HOLD, one clip re-baked live); holds the crowd still (S.peopleFrozen) with one person in the first frame of Idle<Item>[Bite], everyone else hidden (personModel.only), and prints the values to paste back. — exports: openHeldDebug
