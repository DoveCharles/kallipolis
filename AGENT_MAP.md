...
- `serve.py` Dev server with no-store caching: `python3 serve.py [port]`. Also writes `assets/text/index.txt` (every .txt under assets/text/, for the speech loader) on start and whenever it's fetched; `python3 serve.py --index` only writes it. `.claude/launch.json` lists ports 8743–8753.
...
## assets/
- `text/*.txt` Editable data read by `src/core/entries.js`: `text/people/` (one file per list: `about.txt` — notes, trait table, [distribution] — `boynames`, `girlnames`, `unisexnames` (into both), `surnames`, `nicknames`, `moods`, `loves`, `hates`; joined in order under their headings by life/profiles.js PEOPLE_FILES), `cars.txt`, `buildings.txt`, `bees.txt`, `pigeons.txt`, `planes.txt`, `trains.txt`, `tv.txt` (YouTube links or direct video file URLs), `morality.txt`, `text/index.txt` (generated by serve.py; commit it). `text/speech/` (what people say and think, in subfolders as you like: one category per file, named by file — words with forms, conversations with > replies, thoughts, {tags}, world limits, loves/hates; format in `speech/about.txt`; read by src/life/speech-text.js). `dictionary.txt` is unused (replaced by speech/).
...
- `dictionary.js` (6K) Someone talking near the camera now and then says a real line from life/speech-text.js (a reply, if the last line in their group has replies — group.talk, REPLY_WINDOW), synthesized in their babble voice; up to MAX_LINES at once (no limit with babble only as fallback, when people out of hearing stay quiet — people.js), LINE_GAP per conversation (group.quietUntil). — exports: sayLine, lineMouth, stopLine
...
- `voices.js` (14K) People talking babble, Animal Crossing style: — exports: nextSyllable, babble, loudnessOf, exclaim, hearDistance, hearRef
...
- `peopleModel.js` (97K, **big**) assets/models/Person.glb replaces the cuboids once it loads — one rigged figure, drawn for everyone at once as one instanced mesh, flat-shaded. Snack clips (WalkHotdog, IdleCoffeeBite…) are reposed variants of Walk/Idle/Sit1 marked by `base`. WaveLeft and Idle2Left are Wave and Idle2 baked mirror-image (`mirror`), for someone whose right hand is full; WaveLeftBeer… keep the carrying arm of IdleBeer… (`hold`). — exports: PERSON_BAKE_FPS, FIDGETS, FADE_QUICK, CHAT_GAP, CIRCLE_RADIUS, CIRCLE_MAX, PERSON_ARM_SPREAD, PERSON_TRAIT_COLORS …
