...
## assets/
- `*.txt` Editable data read by `src/core/entries.js`: `people.txt` (names, moods, likes, traits — trait syntax at top of file), `cars.txt`, `buildings.txt`, `bees.txt`, `pigeons.txt`, `planes.txt`, `trains.txt`, `tv.txt` (YouTube links or direct video file URLs), `morality.txt`, `dictionary.txt` (real spoken lines).
...
- `dictionary.js` (5K) Now and then someone talking near the camera says something real in among their babble (see audio/voices.js): (the line's `text` rides on what sayLine returns) — exports: sayLine, lineMouth, stopLine
